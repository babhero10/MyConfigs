...
class MapAnalyzer:
    def __init__(self, n_blocks):
        ...
        # Subscribe to the /odom topic to get the rover's current position
    ...
    def check_obstacles(self):
        ...
        if self.rover_position is None:
            rospy.logwarn("Rover position not received yet.")
            return None
        ...
        if obstacle_cells > 0:
            rospy.loginfo(f"Obstacles detected in {obstacle_cells} cells around the rover.")
            return True
        else:
            rospy.loginfo("No obstacles detected around the rover.")
            return False


if __name__ == "__main__":
    rospy.init_node("map_analyzer")  # Initialize the ROS node

    # Define the size of the square region (e.g., 5 x 5 blocks)
    n_blocks = 15
    ...
    while not rospy.is_shutdown():
        # Call the check_obstacles method to analyze the map
